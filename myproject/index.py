...
app.secret_key="dnt tell" # for flash 
	...
	if request.method == "POST":
		...
		if len(data)==0:
			mycur.execute("""insert into students(rollno,name,college,branch,section,email,phno,gender,languages)
				values(%s,%s,%s,%s,%s,%s,%s,%s,%s)""",(rollno,name,college,branch,section,email,phno,gen,lan))
			myconn.commit()
...
def update():
	...
	if request.method == "POST":
		...
		lan=','.join(lan)
		mycur=myconn.cursor()
		mycur.execute("""update students set rollno=%s,name=%s,email=%s,phno=%s,college=%s,branch=%s,section=%s,
			gender=%s,languages=%s where sno=%s""",(rollno,name,email,phno,college,branch,section,gen,lan,id))
		myconn.commit()
		return redirect(url_for('view'))
			...
			with open("static/data/"+name) as f1:
				...
				if len(records[0])!=9:
					...
					for record in records[1:]:
						mycur.execute("""insert into students(rollno,name,college,branch,section,email,phno,gender,languages)
					values(%s,%s,%s,%s,%s,%s,%s,%s,%s)""",(record[0],record[1],record[2],record[3],
